...
fn test_inventory_normalised() {
    let mut s = empty_state();
    s.inventory = [20, 20, 20, 20, 20, 20, 20];
    let v = s.to_state_vector();
    for i in 8..15 {
        assert!((v[i] - 1.0).abs() < 1e-6, "inventory[{}] should be 1.0", i - 8);
    }
}
...
fn test_message_direction_8_encodes_correctly() {
    ...
    let v = s.to_state_vector();
    assert_eq!(v[15], 1.0);
    assert_eq!(v[24], 1.0, "direction 8 bit should be at index 24");
}
...
fn test_vision_resource_capped_at_10() {
    ...
    s.look_tiles = vec![tile];
    let v = s.to_state_vector();
    assert!((v[25] - 1.0).abs() < 1e-6, "food=999 should clamp to 1.0");
}
